...
function Mac({ mac, onKazanan, yeniKonum }) {
      ...
      {["ust", "alt"].map((taraf) => {
        ...
        return (
          <button
            key={taraf}
            type="button"
            className={`agac-oyuncu${kazandi ? " kazanan" : ""}${kaybetti ? " kaybeden" : ""}${onKazanan ? " secilebilir" : ""}${sira != null ? " agac-oyuncu--yeni" : ""}`}
            style={sira != null ? { animationDelay: `${sira * 0.35}s` } : undefined}
            disabled={!onKazanan || !oynanabilir || !oyuncu}
            tabIndex={onKazanan ? 0 : -1}
            onClick={onKazanan ? () => onKazanan(mac, oyuncu) : undefined}
          >
            {oyuncu || "—"}
          </button>
        );
...
export default function AgacGorunum({ bolgeler, sonuclar, turAdlari, sampiyonEtiket, onKazanan, yeniBolge }) {
          ...
          <div className="agac-kolon">
            <div className="agac-cift agac-cift--tek">
              <div className="agac-slot">
                <div className={`agac-sampiyon${sampiyon ? "" : " bos"}`}>{sampiyon || "—"}</div>
              </div>
            </div>
          </div>
        </div>
